...
const AnnouncementEditPage = () => {
  ...
  const getDept = user.departmentId;
      ...
      fetch(`http://localhost:5000/api/announcements/get/${id}`)
        ...
        .then((data) => {
          const announcementData = {
            ...
            time: new Date(data.timestamp._seconds * 1000)
              .toISOString()
              .slice(0, 16),
            title: data.title || "",
  ...
  useEffect(
    () => async () => {
      await fetch(`http://localhost:5000/api/departments/classes/${getDept}`)
        .then((res) => res.json())
  ...
  const handleDelete = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`http://localhost:5000/api/announcements/delete/${id}`, {
          method: "DELETE",
        })
          .then(() =>
            Swal.fire(
          ...
          .then(() => navigate("/faculty/dashboard"));
      }
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (!isEmpty) {
      ...
      console.log(updatedAnnouncement.targetId);
      ...
      formData.append("targetId", updatedAnnouncement.level == "department" ? getDept : updatedAnnouncement.targetId);
        ...
        if (result.isConfirmed) {
          fetch("http://localhost:5000/api/announcements/create", {
            ...
            .then((data) => {
              Swal.fire(
                "Saved!",
                "The announcement has been created.",
                "success"
              );
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <FacultyHeader
        text={isEmpty ? "New Announcement" : "Edit Announcement"}
      />
      <form
        onSubmit={handleSubmit}
        className="space-y-6 bg-white p-8 rounded-xl shadow-lg max-w-4xl mx-auto"
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          {updatedAnnouncement.level === "class" && (
            <div>
              ...
              <select
                name="targetId"
                id="targetId"
                value={updatedAnnouncement.targetId} // <- This should be updatedAnnouncement.targetId
                onChange={handleInputChange}
                className="p-3 border rounded-md w-full"
              >
                <option key="" value="">
          ...
          <div>
            ...
            {filePreview && (
              <div className="mt-4">
                <img
                  src={`http://localhost:5000/api/users/profile?url=${filePreview}`}
                  alt="File Preview"
                  className="max-w-xs"
                />
                <button
