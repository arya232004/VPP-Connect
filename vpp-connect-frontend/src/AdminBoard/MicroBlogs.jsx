import React, { useState, useEffect } from "react";
import { ChevronDown, ChevronLeft, ChevronRight, RotateCw, PlusCircle, Search, TableOfContents } from "lucide-react";
import { useNavigate } from "react-router-dom";
import AdminHeader from "./AdminHeader";

function MicroBlogs() {
    const [selectedCategory, setSelectedCategory] = useState("All");
    const [searchTerm, setSearchTerm] = useState("");
    const [isDropdownOpen, setIsDropdownOpen] = useState(false);
    const [currentPage, setCurrentPage] = useState(1);
    const link = window.location.href;
    const [isFull, setIsFull] = useState(false);
    const itemsPerPage = isFull ? 10 : 4;
    const navigate = useNavigate();
    const [microBlogData, setMicroBlogsData] = useState([]);

    // Sample categories (can be removed as per your requirement)
    const categories = ["All", "Technology", "Science", "Mathematics", "Astronomy"];

    // Fetch microblog data from the API
    useEffect(() => {
        fetch(`http://localhost:5000/api/microblogs/`)
            .then((response) => response.json())
            .then((data) => setMicroBlogsData(data))
            .catch((error) => console.error("Error fetching users:", error));
    }, []);

    useEffect(() => {
        if (link.includes("blogs")) {
            setIsFull(true);
        }
    }, []);

    // Filter microblogs based on search term
    const filteredMicroBlogsBySearch = microBlogData.filter((blog) =>
        blog.content.toLowerCase().includes(searchTerm.toLowerCase()) || blog.username.toLowerCase().includes(searchTerm.toLowerCase())
    );

    // Pagination for MicroBlogs
    const totalPages = Math.ceil(filteredMicroBlogsBySearch.length / itemsPerPage);
    const paginatedMicroBlogs = filteredMicroBlogsBySearch.slice(
        (currentPage - 1) * itemsPerPage,
        currentPage * itemsPerPage
    );

    const handleCategoryChange = (category) => {
        setSelectedCategory(category);
        setIsDropdownOpen(false);
    };

    return (
        <div>
            {/* MicroBlogs Section Header */}
            <div className="flex flex-col gap-1 m-auto md:m-0">
                {isFull ? <AdminHeader text="All Microblogs" /> : <h1 className="text-2xl font-neueMedium md:text-2xl mt-6 mb-4">MicroBlogs</h1>}
            </div>
            <div className="bg-white w-full rounded-2xl p-6">
                {/* Search and Category Dropdown */}
                <div className="flex flex-col md:flex-row justify-between mb-4 gap-3 items-center">
                    {/* Search Bar */}
                    <div className="flex border-2 p-2 rounded-lg border-gray-400 opacity-50 gap-3 items-center px-4 w-full md:w-[54%] mb-2 md:mb-0">
                        <Search size={20} />
                        <input
                            type="text"
                            placeholder="Search MicroBlogs"
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full bg-transparent outline-none text-lg"
                        />
                    </div>

                    {/* Categories Dropdown */}
                    <div className="relative w-full md:w-[32%] z-1">
                        <button
                            onClick={() => setIsDropdownOpen(!isDropdownOpen)} // Toggle dropdown visibility
                            className="flex w-full border-2 p-2 px-4 rounded-lg border-gray-400 opacity-50 gap-3 items-center justify-between"
                        >
                            <h1 className="text-lg">{selectedCategory}</h1>
                            <ChevronDown size={20} />
                        </button>
                        {isDropdownOpen && (
                            <div className="absolute left-0 w-full bg-white shadow-md rounded-lg text-lg">
                                {categories.map((category) => (
                                    <div
                                        key={category}
                                        onClick={() => handleCategoryChange(category)} // Update category on click
                                        className="p-2 px-4 cursor-pointer hover:bg-themegray opacity-50"
                                    >
                                        {category}
                                    </div>
                                ))}
                            </div>
                        )}
                    </div>

                    {/* Add New MicroBlog Button */}
                    <div className="text-sm md:text-lg rounded-xl gap-2 md:gap-2 md:w-[22%] flex justify-between">
                        <button className="border-1 text-[#5d46ac] p-2 rounded-lg flex items-center gap-2 px-1 md:px-5 cursor-pointer"
                            onClick={() => navigate('/admin/microblogs')}>
                            View All <TableOfContents size={30} />
                        </button>
                        <button className="bg-[#5d46ac] text-white p-2 rounded-lg flex gap-2 items-center px-1 md:px-5 cursor-pointer"
                            onClick={() => navigate('/admin/edit-microblogs')}>
                            Add New
                            <PlusCircle size={20} />
                        </button>
                    </div>
                </div>

                {/* Pagination Controls */}
                <div className="flex items-center opacity-50 justify-between mt-4 px-2">
                    <RotateCw size={1} />
                    <div className="flex items-center gap-3">
                        <h3>{currentPage} of {totalPages}</h3>
                        <div className="flex gap-6 items-center">
                            <button
                                onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))}
                                disabled={currentPage === 1}
                            >
                                <ChevronLeft size={20} />
                            </button>
                            <button
                                onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))}
                                disabled={currentPage === totalPages}
                            >
                                <ChevronRight size={20} />
                            </button>
                        </div>
                    </div>
                </div>

                {/* MicroBlog List */}
                <div className="flex flex-col gap-2 mt-2">
                    {paginatedMicroBlogs.length > 0 ? (
                        paginatedMicroBlogs.map((blog, index) => (
                            <div
                                key={index}
                                onClick={() => navigate(`/admin/edit-microblogs/${blog.id}`)} // Navigate to the microblog detail page
                                className="flex items-center p-1 md:p-3 rounded-xl hover:bg-themegray cursor-pointer justify-between w-full"
                            >
                                <div className="flex items-center w-[calc(100%-20px)]">
                                    <img src={blog.profilePic} alt={blog.username} className="p-1 rounded-full mr-2 text-white w-12 h-12" />
                                    <div className="w-full">
                                        <h1 className="text-lg md:text-xl font-neueMedium truncate w-[calc(100%-60px)]">
                                            {blog.username}
                                        </h1>
                                        <div className="flex gap-1 flex-col md:flex-row md:gap-10">
                                            <h1 className="opacity-50 text-xs md:text-base">Likes: {blog.likes.length}</h1>
                                            <h1 className="opacity-50 text-xs md:text-base">Posted on: {new Date(blog.timestamp._seconds * 1000).toLocaleDateString()}</h1>
                                        </div>
                                        <div className="text-sm opacity-70 mt-2" dangerouslySetInnerHTML={{ __html: blog.content }}></div>
                                    </div>
                                </div>
                                <div className="shrink-0">
                                    <ChevronDown size={20} />
                                </div>
                            </div>
                        ))
                    ) : (
                        <h1 className="text-center opacity-50 mt-4">No microblogs found.</h1>
                    )}
                </div>
            </div>
        </div>
    );
}

export default MicroBlogs;
